...
var words = ["amper", "other"]
// var words = ["amper", "hello", "what is", "the weather", "the time",
  ...
  createPredictBtn(){
    ...
    predButton.addEventListener('mousedown', () => {
      ...
      if(Math.max(...exampleCount) > 0){
        ...
        if(exampleCount[0] == 0){
          alert(
            `You haven't added examples for the wake word AMPer`
            )
          return
        }
...
class TextToSpeech{
  ...
  speak(word){
    ...
    if(word == 'amper'){
      ...
      setTimeout(() => {
        // if no query detected after amper is signed
    ...
    if(word != 'amper' && this.currentPredictedWords.length == 0){
      console.log("first word should be amper")
      console.log(word)
      return
    }
